...
`foods.json` resultante (~610 alimentos, muy por debajo de 1 MB).
...
alguna palabra clave de `foods-keywords.json` y no contiene ninguna de las
exclusiones (`baby food`, `fast foods`, `restaurant`, `infant`, `formula`,
`snacks`, `candies`, `school lunch`, `usda commodity`, …). Cuando varias
descripciones matchean la misma palabra clave se priorizan las variantes
**crudas** (`raw`), luego **Foundation** sobre **SR Legacy**, con un máximo de
3 alimentos por palabra clave.

Al ser una coincidencia de **prefijo exacto** (no un buscador), hay dos efectos
conocidos y aceptados:

- Algunas palabras clave no encuentran nada porque USDA nombra el alimento de
  otra forma (p. ej. `zucchini` está en USDA como "Squash, summer, zucchini…",
  no como "Zucchini…"; `coconut` está bajo "Nuts, coconut…"). De las 285
  palabras clave, 251 tuvieron al menos una coincidencia.
- Alguna palabra clave engancha por accidente una descripción que empieza
  igual pero es otro alimento (p. ej. `butter` coincide con "Butterbur, (fuki),
  raw", y `beer` solo encuentra "Beerwurst" — un embutido con cerveza, no la
  bebida). Se ha revisado la selección final a mano: estos casos están
  traducidos por lo que realmente son (`fuki`, `salchicha de cerveza`), no por
  el nombre de la palabra clave que los enganchó.
...
solo rellena los `sourceRef` que faltan. La primera versión de las 610
traducciones actuales se redactó y se revisó a mano (nombre en español tal
como lo escribiría alguien de casa en una receta: singular, minúsculas, sin
jerga de USDA ni marcas), sin depender de ningún servidor local. Cualquier
corrección futura se hace editando este JSON directamente, nunca el prompt del
script ni `foods.json`.
