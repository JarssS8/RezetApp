import { z } from 'zod'
import { McpProfileSchema } from './common'
...
export const ApiTokenCreateSchema = z.strictObject({ name: z.string().trim().min(1).max(60), scopes: z.array(ApiScopeSchema).min(0).max(API_SCOPES.length), mcpProfile: McpProfileSchema.default('basic') })
