  ...
  return (
    <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
      <span className="flex items-baseline gap-1.5">
        <span className="tabular font-display text-3xl">{nf.format(perServingKcal)}</span>
      ...
      <span className="tabular text-xs text-text-2">
        {nf.format(totalKcal)} {t('kcalUnit')} {t('detail.total')}
      </span>
      <span className="tabular text-xs text-text-2">
        {per100gKcal !== null ? `${nf.format(per100gKcal)} ${t('kcalUnit')} ${t('detail.per100g')}` : '—'}
      </span>
    </div>
  )
}
