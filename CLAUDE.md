...
supabase/
  ...
  functions/   recognize-pantry-item (Gemini vision for pantry photo add; household + size/type check, 50/day quota),
               ...
               send-member-notifications (pg_cron, hourly at minute 5; the `expiring`/`cook_turn`/`log_reminder`
               push notices from `member_notify_pref` — quiet hours apply here, unlike timers — at most once per
               member per day via `member_notice_log`; due-time decision is pure in `due.ts`),
...
- **`household.turns_enabled` is not admin-gated, in the database or the UI** — any household member can flip it, the same as `household.name`. That is intentional, not an oversight: this repo reserves the admin gate (`profile.isAdmin`) for membership actions — inviting, promoting, removing — not for a purely informational switch. Whoever tightens it later should know it was a deliberate choice, made in `supabase/migrations/20260921100200_rezet_turns.sql`.
...
- **The Today dashboard layout is always normalized before painting (`domain/dashboard.ts::normalizeLayout`), and that function never throws** — a layout saved by a future or otherwise-mismatched version of the app (unknown widget ids, invalid sizes, JSON that's neither an array nor an object) falls back to the default layout instead of leaving Today blank on open. A widget the member turns off is stored as `on: false` rather than removed from the list, on purpose: the spec said both "absent means hidden" and "new widgets are appended, visible", which can't both hold — with "hidden = absent", the normalizer would treat a turned-off widget as a newly-added one and re-add it, and the user could never keep anything off.
- **`pointerEvents: 'none'` does not disable a control.** It blocks the mouse, not keyboard focus or Enter/Space on a native `<button>` — someone tabbing through can still activate a control "disabled" only this way. This repo has hit it twice: it's noted as unfixed in `MemberSheet.tsx` (`docs/superpowers/plans/2026-09-20-fundacion-de-miembro-pendientes.md`) and it recurred in the dashboard-widgets phase (`DashboardEditSheet.tsx`'s size segmented control). `Pressable` already took a real `disabled` prop before this phase; `SegmentedControl` gained one in the dashboard-widgets phase so it reaches the native `disabled` attribute too — use that instead of wrapping a control in `pointerEvents: 'none'`.
...
- Edge Function secrets `TIMER_CRON_SECRET` (must equal the Vault secret `timer_cron_secret`) and `APP_ORIGIN` are set in the dashboard, not in the repo. `send-timer-notifications`, `cleanup-orphan-photos` and `send-member-notifications` all refuse to run without `TIMER_CRON_SECRET` (503) since 1.8.1 (the latter reuses the same secret rather than minting its own).
...
- **`MemberSheet.tsx`'s edit lock still uses `opacity` + `pointerEvents: 'none'`** (`docs/superpowers/plans/2026-09-20-fundacion-de-miembro-pendientes.md`), so a keyboard user can still type into fields the mouse can't reach. Left as-is on purpose in the dashboard-widgets phase — not this phase's scope — but the fix is no longer blocked on a missing primitive: `Pressable` already took a real `disabled` prop before this phase, and `SegmentedControl` gained one, added for `DashboardEditSheet.tsx`, so `MemberSheet.tsx` just needs to switch its controls to it.
...
Already done, despite what older docs in this repo say: real Supabase backend + RLS, Google/Apple/Passkey auth with multi-user households + invites, PWA (`vite-plugin-pwa`, installable), background timer notifications via real Web Push (Edge Function + `pg_cron`, not local notifications), "Foto" pantry add via `recognize-pantry-item` (`GEMINI_API_KEY` is set in `app_secret`; model `gemini-3.6-flash` — `gemini-2.0-flash` is shut down, so don't revert to it), and deployed live at `rezet.jarsss8.es`. Also done, closing what were Fases 4, 5 and 7 of `docs/superpowers/plans/2026-09-21-avisos-gustos-turnos.md`: per-member notification preferences with quiet hours (cook timers exempt, see `NotifySheet.tsx`); per-member recipe likes/dislikes with a household-visible aggregate and per-voter breakdown (`RecipeDetail.tsx`); "Para ti" suggestions on Today, scored by `domain/suggestions.ts`; and opt-in, off-by-default cook/shopping shifts (`TurnsSheet.tsx`) that are purely informational. Also done, closing `.superpowers/sdd/2026-09-23-dashboard-de-widgets/` (fase 3, "dashboard de widgets"): Today paints from a per-member layout (`member_dashboard.layout`, one row per member, `domain/dashboard.ts` owns the catalogue/normalization/move logic) instead of a fixed block list; any widget — including the calorie ring — can be turned off; the layout follows a member to any device; `DashboardEditSheet.tsx` reorders by drag or by up/down buttons, both driving the same `moveWidget` and announced over `aria-live`; and `whose_turn` only exists in the grid and the customize catalogue while `household.turns_enabled` is on.
