...
- **Security "Fase B" is pending.** Clients still hold INSERT on `household_invite` so cached pre-1.7 PWAs keep working; the `household_invite_server_mint_trg` trigger rewrites anything inserted that way (and, since `20260918110000_rezet_admin_only_invite_trigger_and_select.sql`, requires the inserting user to be an admin of that household, same as `create_invite()`). Once installed clients have updated: revoke that INSERT, drop its policy and the trigger, and drop the now-unused function `private.force_server_minted_invite()`. The old dependency risk here is already resolved — `create_invite()`/`revoke_invite()` (`20260918110100_rezet_create_invite_independent_of_trigger.sql`) set `created_by`/`expires_at` explicitly in their own insert and no longer rely on the trigger to fill them in, so Phase B dropping it will not make new codes unredeemable; a PGlite test simulates Phase B end to end (`migrations.test.ts`, "create_invite y redeem_invite siguen funcionando si se simula la Fase B"). Phase B also has to rewrite three migration-bank tests that assert the direct insert still works (`migrations.test.ts:180`, `:209`, `:254`) and drop the hand-rolled revoke inside the Phase B simulation test (`:289`) — CI's `test` job gates every deploy and tag, so leaving them red blocks the whole release. Plan: `docs/superpowers/plans/2026-09-17-security-fixes.md`, section "Fase B".
