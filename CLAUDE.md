...
- **`MemberSheet.tsx`'s edit lock still uses `opacity` + `pointerEvents: 'none'`** (`docs/superpowers/plans/2026-09-20-fundacion-de-miembro-pendientes.md`), so a keyboard user can still type into fields the mouse can't reach. Left as-is on purpose in the dashboard-widgets phase — not this phase's scope — but the fix is no longer blocked on a missing primitive: `Pressable` and `SegmentedControl` both now take a real `disabled` prop (added for `DashboardEditSheet.tsx`), so `MemberSheet.tsx` just needs to switch its controls to it.
